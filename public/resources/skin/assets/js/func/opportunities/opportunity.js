        ...
        success: function(e) {
            ...
            if (e.status == 'ok') {
                ...
                $.each(data_export.data, function(key, val) {
                    ...
                    var over_view = truncate(val.over_view, 200);
                    job_location = truncate(val.location_job, 80);
                    contract_length = truncate(val.contract_length, 80);




                    // item_divs += '<div class="pricing-block col-lg-6 col-md-6 col-sm-12">\n' +
                    //     '					<div class="inner-box" style="background-image:url(https://images.pexels.com/photos/1618269/pexels-photo-1618269.jpeg?auto=compress&cs=tinysrgb&dpr=2&h=650&w=940)">\n' +
                    //     '						<div class="price"><span>' + title + '</span></div>\n' +
                    //     '						<h4>' + contract_length + ' in ' + job_location + '</h4>\n' +
                    //     '						<div class="title">' + over_view + '</div>\n' +
                    //     '						<div class="btn-one-outer"><a href="opportunity-view?ref=' + val.record_id + '" class="theme-btn btn-style-one" type="submit"><span class="txt"> More details</span></a></div>\n' +
                    //     '					</div>\n' +
                    //     '				</div>';
                    
                    item_divs +='<div class="col-lg-4 col-md-6">' +
                    '						<div class="single-blog-item style-post">' +
                    '							<a href="opportunity-view?ref=' + val.record_id + '">' +
                    '								<img src="/resources/skin/images/main-slider/pexels-cytonn-photography-955388.jpg" alt="Image">' +
                    '							</a>' +
                    '	' +
                    ...
                    '									<li>' +
                    '										<a href="opportunity-view?ref=' + val.record_id + '">Term: ' + contract_length + ' in ' + job_location + '</a>' +
                    '									</li>' +
                    '								</ul>' +
                    '								<h3>' +
                    '									<a href="opportunity-view?ref=' + val.record_id + '">' + title + '</a>' +
                    '								</h3>' +
                    '	' +                    
                    '							</div>' +
                    '						</div>' +
                ...
                if (e.messages == 'No data yet.') {
                    $('#results-div').html('No open positions at the moment.');
                }
                $(".ajaxDataloader").html("");
                $(".ajaxDataloader").css("visibility", "hidden");

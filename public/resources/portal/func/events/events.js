        ...
        success: function(e) {
            ...
            if (e.status == 'ok') {
                ...
                $.each(data_export.data, function(key, val) {
                    ...
                    var age_bracket_data = val.age_brackets;
                    ...
                    $.each(age_bracket_data, function(key, val) {
                        console.log('val', val);

                        age_brackets += val.description + ' | ';
                    })

                    var service_pill = '<span class="badge badge-light">' + services + '</span>';
                    //$('#display-trainer-services').html(service_pill); client-vw-book-event-more-details
                    item_divs += '<div class="col-12 col-lg-4 col-xl-4">' +
                        '<div class="card">' +
                        '<img src="' + val.thumbnail + '" class="card-img-top" alt="...">' +
                        '<div class="card-body">' +
                        '<h5 class="card-title mb-0">' + val.name + '</h5>' +
                        '<p class="mb-0"><strong>From:</strong> ' + val.startdate + ' <strong>to:</strong> ' + val.enddate + '</p>' +
                        '<p class="mb-0"><strong>Location:</strong> ' + val.location +'</p>' +
                        '<p class="mb-0"><strong>Status:</strong> ' + val.unit_ui_display +'</p>' +
                        '<p class="mb-0"><strong>Services:</strong> ' + service_pill +'</p>' +
                        '<p class="mb-0"><strong>Age Bracket:</strong> ' + age_brackets +'</p>' +
                        '<hr>' +
                        '<a href="/client-vw-book-event-more-details?record-id=' + val.record_id + '" class="btn btn-light">More Details</a>' +
                        '</div>' +
                        '</div>' +
                        '</div>';
...
function get_ongoing_services() {
    var b = {
        'token': localStorage.token,
    };
    $.ajax({
        type: "post",
        url: "/get-client-all-sessions-ongoing",
        data: b,
        dataType: "json",
        beforeSend: function() {
            $('.loading-div').removeClass('hide-me');

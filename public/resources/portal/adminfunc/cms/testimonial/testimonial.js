...
$("#message-form").submit(function (c) {
    ...
    if (question == 0) {
        $('#faq-question').addClass('is-invalid-selection-div');
        ...
        $('html, body').animate({
            scrollTop: $('#faq-question').focus().offset().bottom - 5
        }, 2000);
        return false;
    } else {
        $('#faq-question').removeClass('is-invalid-selection-div');
    }
...
$(document).on("click", ".actionbutton", function () {
    ...
    let attr_id = $(this).attr('attr-id');
    let attr_act = $(this).attr('attr-act');
    update_testimonials_details(attr_id,attr_act);
});


function update_testimonials_details(attr_id,attr_act) {
    

    var b = {
        "testimonials_id": attr_id,
        "attr_act": attr_act,
        'token': localStorage.token,
    };
    $.ajax({
        type: "post",
        url: "/update-testimonials",
        data: b,
        dataType: "json",
        beforeSend: function () {
            alert_title = 'working...';
        ...
        success: function (e) {
            if (e.status == 'ok') {
                ...
                //console.log('faq data1',e);
                let faq_data = e.data;
                let faq_data_nest1 = faq_data;

                $.each(faq_data_nest1, function(key, val) {
                    console.log(val[0]);
                    
                $('#faq-id').val(val[0].record_id);
                $('#faq-question-ed').val(val[0].question);
                $('#msg-extra-info-ed').val(val[0].answer);
                $('#new-faq-modal-ed').modal('show');
...
function fetch_faq_details(attr_id) {
...
$("#message-form-ed").submit(function (c) {
    ...
    var question = $("#faq-question-ed").val();
